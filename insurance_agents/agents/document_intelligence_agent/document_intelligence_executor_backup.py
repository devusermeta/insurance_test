...
class DocumentIntelligenceExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            result = await self._process_claim_documents_with_a2a(user_input)
    ...
    async def _handle_new_workflow_document_processing(self, task_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # STEP 5: Call intake clarifier via A2A after document processing
            await self._call_intake_clarifier_via_a2a(claim_id)
    ...
    async def _call_intake_clarifier_via_a2a(self, claim_id: str) -> None:
        """
        Call intake clarifier via A2A after document processing is complete
        This implements your workflow requirement for A2A communication
        """
        try:
            self.logger.info(f"📞 Calling intake clarifier via A2A for claim: {claim_id}")
            
            # Create message for intake clarifier
            message = f"""Document Intelligence processing complete for claim {claim_id}.
            
Documents have been processed and stored in extracted_patient_details container.
Please verify the extracted data against the original claim details and update the claim status accordingly.

Claim ID: {claim_id}
Container: extracted_patient_details
Action Required: Verify patient data and update claim status"""

            # Send A2A message to intake clarifier
            await self.a2a_client.send_message(
                target_agent="intake_clarifier",
                message=message
            )
            
            self.logger.info(f"✅ Successfully called intake clarifier via A2A for claim {claim_id}")
            
        except Exception as e:
            self.logger.error(f"❌ Error calling intake clarifier via A2A for claim {claim_id}: {e}")
            raise  # Re-raise to ensure the error is handled upstream

    async def _process_claim_documents_with_a2a(self, task_text: str) -> Dict[str, Any]:
        """
        Clean implementation of your workflow with A2A communication
        """
        try:
            # Check if this is a new workflow request with claim details
            if self._is_new_workflow_claim_request(task_text):
                self.logger.info("🔄 Using NEW WORKFLOW with A2A communication")
                result = await self._handle_new_workflow_document_processing(task_text)
                return result
            else:
                # Fallback to legacy processing
                self.logger.info("🔄 Using legacy processing")
                return await self._process_document_intelligence_task(task_text)
                
        except Exception as e:
            self.logger.error(f"❌ Error in claim documents processing with A2A: {e}")
            return {"status": "error", "response": f"Processing failed: {str(e)}"}

# Create the executor instance - using the correct class name
# DocumentIntelligenceExecutor is already defined above, no need for alias

print("📄 Document Intelligence Executor loaded successfully!")
print("✅ A2A compatible version with A2A communication to intake clarifier")
